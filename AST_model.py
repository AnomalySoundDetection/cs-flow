...
class ASTModel(nn.Module):
    ...
    """
    def __init__(self, label_dim=527, fstride=10, tstride=10, input_fdim=128, input_tdim=1024, target_size=32, imagenet_pretrain=True, audioset_pretrain=False, model_size='base384', verbose=True):
    ...
    def forward(self, x):
        ...
        x = self.v.norm(x)
        x = x.permute(0, 2, 1)
        x = nn.functional.interpolate(x.unsqueeze(2), size=(self.target_size, self.target_size), mode='bilinear', align_corners=False)
...
# Load Pretrain Model
################################################################
def load_extractor(tdim=1024, fdim=128, target_size=32):
    
    # download pretrained model in this directory
    os.environ['TORCH_HOME'] = './' 
    
    # create an AST model
    extractor = ASTModel(input_tdim=tdim, input_fdim=fdim, audioset_pretrain=True, target_size=target_size)
